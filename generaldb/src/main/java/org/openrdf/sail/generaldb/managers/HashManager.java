...
public class HashManager extends ManagerBase {
   ...
   * List<RdbmsValue> values = new ArrayList<RdbmsValue>(getChunkSize()); synchronized (assignIds) {
   * throwException(); if (value.isExpired(version)) { Map<Long, Number> map = new HashMap<Long,
   * Number>(getChunkSize()); //FIXME careful here!! my additions!!!
   * 
   * //System.out.println("taken not null"); if((value instanceof RdbmsLiteral)) {
   * System.out.println("In assignId, found a literal "+value.toString()); URI datatype =
   * ((RdbmsLiteral)value).getDatatype(); if(!XMLDatatypeUtil.isGeoSpatialDatatype(datatype)) {
   * System.out.println("In assignId, no geospatial"); //the only original //if (taken != null) {
   * values.add(value); assignIds(values, map);
   * 
   * } else { System.out.println("In assignId, geospatial found, therefore no additions"); } } }
   * 
   * } for (RdbmsValue v : values) { insert(v); } }
   */
  // backing up
  ...
  /*
   * @Override public void flush() throws SQLException, InterruptedException { throwException();
   * List<RdbmsValue> values = new ArrayList<RdbmsValue>(getChunkSize()); Map<Long, Number> map =
   * new HashMap<Long, Number>(getChunkSize()); RdbmsValue taken = queue.poll(); while (taken !=
   * null) { values.clear(); values.add(taken); synchronized (assignIds) {
   * //System.out.println("invoking assignIds"); assignIds(values, map); } for (RdbmsValue v :
   * values) { insert(v); } taken = queue.poll(); if (taken == closeSignal) { queue.add(taken);
   * taken = null; } } synchronized (working) { values.clear(); synchronized (assignIds) {
   * throwException();
   * 
   * 
   * 
   * //System.out.println("taken not null"); if((this.taken instanceof RdbmsLiteral)) {
   * System.out.println("In flush, found a literal "+this.taken.toString()); URI datatype =
   * ((RdbmsLiteral)this.taken).getDatatype(); if(!XMLDatatypeUtil.isGeoSpatialDatatype(datatype)) {
   * System.out.println("In flush, no geospatial"); //the only original //if (taken != null) {
   * values.add(this.taken); assignIds(values, map);
   * 
   * } else { System.out.println("In flush, geospatial found, therefore no additions"); } this.taken
   * = null; } } for (RdbmsValue v : values) { insert(v); } } super.flush(); }
   */
  ...
  void lookupThread(Object working, Object assignIds) throws InterruptedException,
   ...
   * List<RdbmsValue> values = new ArrayList<RdbmsValue>(getChunkSize()); Map<Long, Number> map =
   * new HashMap<Long, Number>(getChunkSize()); taken = queue.take(); for (; taken != closeSignal;
   * taken = queue.take()) { synchronized (working) { values.clear(); synchronized (assignIds) {
   * //FIXME VERY CAREFUL HERE!!MY ADDITIONS!! if(taken!=null) {
   * //System.out.println("taken not null"); if((taken instanceof RdbmsLiteral)) {
   * System.out.println("taken a literal "+taken.toString()); URI datatype =
   * ((RdbmsLiteral)taken).getDatatype(); if(!XMLDatatypeUtil.isGeoSpatialDatatype(datatype)) {
   * System.out.println("no geospatial here"); //the only original //if (taken != null) {
   * values.add(taken); assignIds(values, map); taken = null; } else {
   * System.out.println("Geospatial, therefore no additions"); } } } } for (RdbmsValue v : values) {
   * insert(v); } } } }
   */

  // FIXME i am not supposed to be public!!!
  // modified to use it at RdbmsTripleRepository -> insert
  public void assignIds(List<RdbmsValue> values, Map<Long, Number> map) throws SQLException,
  ...
  /**
   * FIXME obviously my addition doesn't affect the other functions in any way - just added variable
   * GeoIds for my convenience after all it seems it is not needed
   */
  /*
   * public void assignGeoSpatialIds(RdbmsValue objectLiteral) throws SQLException,
   * InterruptedException { String query = "SELECT id FROM manosgeofull2_values"; try { Connection
   * conn = triples.getConnection(); PreparedStatement stmt = conn.prepareStatement(query);
   * 
   * //Collection<ColumnVar> proj = qb.getProjections(); RdbmsBindingIteration result = new
   * RdbmsBindingIteration(stmt); //result.setProjections(proj); result.setBindings(bindings);
   * result.setValueFactory(vf); result.setIdSequence(ids); return result; } catch (SQLException e)
