...
public abstract class GeneralDBBindingIteration extends
  ...
  private static Logger logger = LoggerFactory
      .getLogger(org.openrdf.sail.generaldb.iteration.GeneralDBBindingIteration.class);
  ...
  // protected HashMap<String, Integer> sp_ConstructIndexesAndNames = new HashMap<String,
  // Integer>();
  protected HashMap<GeneralDBSpatialFuncInfo, Integer> sp_ConstructIndexesAndNames =
  ...
  //
  // protected HashMap<String, Integer> sp_IntPropertiesIndexesAndNames = new HashMap<String,
  // Integer>();
  //
  // protected HashMap<String, Integer> sp_BoolPropertiesIndexesAndNames = new HashMap<String,
  // Integer>();
  //
  // protected HashMap<String, Integer> sp_StringPropertiesIndexesAndNames = new HashMap<String,
  // Integer>();

  //

  public GeneralDBBindingIteration(PreparedStatement stmt) throws SQLException {
    super(stmt);
  }
  ...
  protected BindingSet convert(ResultSet rs) throws SQLException {
    // / debug
    /*
     * for(int i=1; i<12;i++) { Object o = rs.getObject(i); if (o instanceof byte[] ) { byte[] label
     * = rs.getBytes(i); int srid = rs.getInt(i + 1); GeneralDBPolyhedron g =
     * vf.getRdbmsPolyhedron(114, StrabonPolyhedron.ogcGeometry, label, srid);
     * System.out.println(i+": "+g.getPolyhedronStringRep()); } else if (o instanceof Blob ) { Blob
     * labelBlob = rs.getBlob(i); byte[] label = labelBlob.getBytes((long)1,
     * (int)labelBlob.length()); int srid = rs.getInt(i + 1); GeneralDBPolyhedron g =
     * vf.getRdbmsPolyhedron(114, StrabonPolyhedron.ogcGeometry, label, srid);
     * System.out.println(i+": "+g.getPolyhedronStringRep()); } else
    ...
    for (GeneralDBSpatialFuncInfo construct : sp_ConstructIndexesAndNames.keySet()) {
      ...
      switch (construct.getType()) {
        case BOOLEAN:
          value =
              createBooleanGeoValueForSelectConstructs(rs,
                  sp_ConstructIndexesAndNames.get(construct));
          break;
        case DOUBLE:
          value =
              createDoubleGeoValueForSelectConstructs(rs,
                  sp_ConstructIndexesAndNames.get(construct));
          break;
        case INTEGER:
          value =
              createIntegerGeoValueForSelectConstructs(rs,
                  sp_ConstructIndexesAndNames.get(construct));
          break;
        case STRING:
          value =
              createStringGeoValueForSelectConstructs(rs,
                  sp_ConstructIndexesAndNames.get(construct));
          break;
        case WKT:
          value =
              createWellKnownTextGeoValueForSelectConstructs(rs,
                  sp_ConstructIndexesAndNames.get(construct));
          break;
        case WKTLITERAL:
          value =
              createWellKnownTextLiteralGeoValueForSelectConstructs(rs,
                  sp_ConstructIndexesAndNames.get(construct));
          break;
        case URI:
          value =
              createURIGeoValueForSelectConstructs(rs, sp_ConstructIndexesAndNames.get(construct),
                  construct.isSRIDFunc());
          break;
        default:
  ...
  protected RdbmsResource createResource(ResultSet rs, int index) throws SQLException {
  ...
  protected RdbmsValue createValue(ResultSet rs, int index) throws SQLException {
    Number id = ids.idOf(rs.getLong(index));
    if (ids.isLiteral(id)) {
  ...
  /**
   * Creates a geospatial value from the given result set and index position. When projecting on a
   * geospatial value, we get also its SRID, and its datatype, so that we are able to assign that
   * datatype to the new value.
   * 
   * @param rs the current result set over which we are iterating
   * @param index the index position to start reading from index + 1: geospatial value (in binary)
   *        index + 2: SRID index + 3: datatype
   * @return
   * @throws SQLException
   */
  protected abstract RdbmsValue createGeoValue(ResultSet rs, int index) throws SQLException;

  protected abstract RdbmsValue createWellKnownTextGeoValueForSelectConstructs(ResultSet rs,
      int index) throws SQLException;

  protected abstract RdbmsValue createWellKnownTextLiteralGeoValueForSelectConstructs(ResultSet rs,
      int index) throws SQLException;
  ...
  protected RdbmsResource createURIGeoValueForSelectConstructs(ResultSet rs, int index,
      boolean sridTransform) throws SQLException {
    String uri = null;
