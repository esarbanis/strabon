...
import org.openrdf.query.algebra.evaluation.function.spatial.*;
...
import static org.openrdf.sail.generaldb.algebra.GeneralDBColumnVar.*;
...
// implements QueryOptimizer //removed it consciously
{
  ...
  // private Map<String, GeneralDBSqlExpr> constructsForSelect = new HashMap<String,
  // GeneralDBSqlExpr>();
  ...
  /**
   * This happens when both sides of the union have the same variable name with an implied value.
   */
  private void addProjectionsFromUnion(GeneralDBSelectQuery query, GeneralDBUnionItem union) {
    for (GeneralDBColumnVar var : union.getSelectColumns()) {
      if (!query.hasSqlSelectVarName(var.getName())) {
        ...
        proj.setStringValue(coalesce(new GeneralDBURIColumn(var), new GeneralDBBNodeColumn(var),
            new GeneralDBLabelColumn(var), new GeneralDBLongLabelColumn(var),
            new GeneralDBLongURIColumn(var)));
  ...
  public void meet(Join node) throws RuntimeException {
    ...
    /**
     * XXX cheating - used 'reference' to find the final select query and attach the constructs
     * information on it This change was made before altering the spatial joins operation
     */
    reference = left;
  }
  ...
  public void meet(StatementPattern sp) {
    ...
    /**
     * Effort here to recover possible elements located in group by so that I order the results
     * according to them
     */
    ...
    for (GeneralDBColumnVar var : new GeneralDBColumnVar[] {s, p, o, c}) {
      ...
      if (vars.containsKey(var.getName())) {
        ...
        if (var.getColumn().equals("obj") && previousSpatialArg != null) {
          from.addFilter(new GeneralDBSqlEq(new GeneralDBIdColumn(var), new GeneralDBIdColumn(
              previousSpatialArg)));
      ...
      if (!var.isHiddenOrConstant() && value == null) {
        ...
        if (geoNames.contains(var.getName())) {
          ...
          // 13/09/2011 my addition in order to create a spatial join in the meet(Filter) call that
          // will follow
          previousAlias = var;

          // add the corresponding datatype (see {@link
          // GeneralDBValueJoinOptimizer.GeneralDBLabelColumn})
          proj.setDatatype(new GeneralDBDatatypeColumn(var));
        } else {
          proj.setStringValue(coalesce(new GeneralDBURIColumn(var), new GeneralDBBNodeColumn(var),
              new GeneralDBLabelColumn(var), new GeneralDBLongLabelColumn(var),
  ...
  public void meet(Filter node) throws RuntimeException {
    /**
     * XXX 21/09/2011 addition for spatial joins Ekmetalleyomai to gegonos oti exw 'fytepsei' sta
     * embolima filters twn joins mia statement pattern me to onoma -dummy- Etsi katalabainw pote to
     * Filter dn brisketai sto where clause
     */
    if (node.getArg() instanceof StatementPattern) {
      ...
      if (st.getSubjectVar().getName().equals("-dummy-")) {
        ...
        // will probably be TWO contents at most in all cases concerning spatial filters here - have
        // to make sure of that
        // Don't know which one of the spatial variables is bound with the upper join! Therefore, I
        // altered the code to check
        ...
        if (allVars.size() > 1) {
          for (Var var : allVars) {
            if (var.getName().equals(previousAlias.getName())) {
              ...
              // }
              // else //DEFAULT CASE. The only case differentiating from this one is
              // buffer(?spatialVar,?thematicVar)
              // {
              ...
              from.addFilter(new GeneralDBSqlEq(new GeneralDBIdColumn(colVar),
                  new GeneralDBIdColumn(previousAlias)));
        ...
        //
        // //assuming that only one var will reach this case
        // from.addFilter(new GeneralDBSqlEq(new GeneralDBIdColumn(colVar), new
        // GeneralDBIdColumn(previousAlias)));
        //
        // //Copying functionality from meet(StatementPattern)
        // GeneralDBSelectProjection proj = new GeneralDBSelectProjection();
        // proj.setVar(colVar);
        ...
        // What i wanted to do is replace a node with 2 GeneralDBSelectQueries that derived from the
    ...
    else // DEFAULT BEHAVIOR (Enhanced with check for duplicates)
    {
      ...
      for (TupleExpr sfilter : this.spatialJoins) {
        ...
        if (tmpExpr.equals(dupFunctionCall)) {
          ...
          // //If I do reach this point, the former 'child' argument will be a traditional Join
          // operator
        ...
        if (!dup) {
          for (ValueExpr expr : flatten(node.getCondition())) {
            try {
              ...
              // XXX removed because I followed a different approach. May come in handy later on.
              // 09/09/11
              // if(node.getCondition() instanceof FunctionCall)
              // {
      ...
      /**
       * Used a single function to iterate the query graph and try to locate potential aggregates
       * present in HAVING instead of performing multiple loops
       */
      examineHaving(node);
      ...
      // {
      // GroupElem groupElem = new GroupElem("havingCondition"+(havingID++)+"-aggregateInside-", new
      // Avg(cmp.getLeftArg()));
    ...
    if (function instanceof SpatialRelationshipFunc || function instanceof SpatialConstructFunc
      ...
      for (ValueExpr arg : allArgs) {
        ...
        if (arg instanceof Var) {
          if ((!(function instanceof BufferFunc) && !(function instanceof TransformFunc))
              || argNo != 2) {
            ...
            /**
             * XXX TRICK TO SPECIFY THAT THE VAR IS SPATIAL: I will alter the var's name and set it
             * back to the original in the LabelColumn's Constructor!
             * 
             * Only reason to avoid this step: FunctionCall contained in Order and order is to be
             * evaluated by java code and not sql No measure taken so far
             */
    ...
    while (iter.hasNext()) {
      ...
      { // if the expr is grounded we are going to evaluate it in Java
        ...
        if (!evaluateInJava(expr) && !varInBind(expr)) {
          ...
          if (function instanceof SpatialPropertyFunc
        ...
        } else if (!varInBind(expr)) // Union (or Extent) is used as an aggregate function on Select
                                     // Clause!
        {
      ...
      // One way to do it: Check for label column in children of expression. If none exists: No way
      // sth spatial is involved!
      ...
      if (this.namesInGroupBy != null) // && !(expr instanceof FunctionCall))
      {
        if (this.namesInGroupBy.contains(elem.getName())) {
          if (expr instanceof FunctionCall) {
            /**
             * No need to produce the sql part twice What is more, if I try to produce it again, the
             * vars will have the ?spatial part corrected
             */
  ...
  /**
   * Checks whether the given value expression contains only grounded terms (constants).
   * 
   * This should work for the spatial case, but I am not 100% sure whether it is going to work for
   * whole SPARQL 1.1.
   * 
   * @param funcExpr
   * @return
  ...
  /**
   * XXX 23/11/11 Commented because I now aim to evaluate any function calls present in SELECT with
   * JTS in the case they contain an aggregate function
   */
  // @Override
  // public void meet(Extension node) throws RuntimeException
  // {
  ...
  /**
   * Function used recursively to specify whether the function call present in the select clause
   * contains an aggregate of the form strdf:union(?aggrValue) or strdf:intersection(?aggrValue).
   * 
   * @param expr
    ...
    if (expr instanceof FunctionCall) {
      ...
      if ((!(function instanceof UnionFunc) || !(((FunctionCall) expr).getArgs().size() == 1))
        ...
        for (int i = 0; i < ((FunctionCall) expr).getArgs().size(); i++) {
          ...
          // noUnionPresent = noUnionPresent ^ evaluateInJava(((FunctionCall)
          // expr).getArgs().get(i));
          aggregatePresent =
              aggregatePresent || evaluateInJava(((FunctionCall) expr).getArgs().get(i));
        }
   ...
   *
   * @param expr
   * @return true if the variable occurs inside a BIND clause false otherwise
   */
  ...
  /**
   * FIXME uncomment if you need sorted results based on the group by's contents!!! Commented for
   * now because I am not messing with Sesame's native approach to the issue
   */
  @Override
  ...
  /***
   * added for spatial join support TODO perhaps I will need to elaborate and separate the Function
   * Calls in more cases when I add the rest of the Constructs, Properties etc.
   */
  private Set<Var> retrieveVars(ValueExpr expr) {
    ...
    if (expr instanceof FunctionCall) {
      ...
      for (ValueExpr arg : ((FunctionCall) expr).getArgs()) {
        argNo++;
        if (arg instanceof Var) {
          // if(!((FunctionCall)expr).getURI().equals("http://strdf.di.uoa.gr/ontology#buffer") ||
          // argNo!=2 )
          // {
  ...
  /**
   * Used to find out whether the Filter expr we are currently visiting is located inside a HAVING
   * clause.
   * 
   * From what I have seen so far, it seems that if this is the case, its arg (or one of the
   * following FILTER args in the case of disjunction/conjunction) will be an Extension expr
   * followed by a GROUP expr
   */
        ...
        if (expr.getCondition() instanceof Compare) {
          ...
          if (cmp.getLeftArg() instanceof FunctionCall) {
            ...
            if (!found) {
              ...
              GroupElem groupElem =
                  new GroupElem("havingCondition" + (havingID++) + "-aggregateInside-", toBeEntered);
              this.referenceGroupBy.addGroupElement(groupElem);
            }
    ...
    for (ValueExpr expr : func.getArgs()) {
      if (expr instanceof FunctionCall) {
        ...
        if (((function instanceof UnionFunc) && (((FunctionCall) expr).getArgs().size() == 1))
            ...
            || (function instanceof ExtentFunc)) {
          GroupElem groupElem =
              new GroupElem("havingCondition" + (havingID++) + "-aggregateInside-", new Avg(expr));
          this.referenceGroupBy.addGroupElement(groupElem);
          continue;
        } else {
