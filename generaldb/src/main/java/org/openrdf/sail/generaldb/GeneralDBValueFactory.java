...
import org.openrdf.sail.rdbms.model.*;
 ...
 * {@link RdbmsValue}s behaviour similar to the default {@link Value} implementation with the
...
public class GeneralDBValueFactory extends ValueFactoryBase {
  ...
  public GeneralDBPolyhedron getRdbmsPolyhedron(Number num, String datatype, byte[] wkb, int srid) {
    ...
    try {
      if (wkb != null) {
        return new GeneralDBPolyhedron(id, literals.getIdVersion(), vf.createURI(datatype), wkb,
            srid);
      }
    } catch (IOException e) {
      e.printStackTrace();
  ...
  /**
   * This function is called only for SELECT constructs, thus we do not create an id for the
   * resulting geometry because we don't push it in the database since there is small possibility to
   * meet this geometry in the future.
   ...
   * {@link and MonetDBBindingIteration.createWellKnownTextGeoValueForSelectConstructs}, {@link and
   * MonetDBBindingIteration.createWellKnownTextLiteralGeoValueForSelectConstructs}
   * 
   * @param datatype
   * @param wkb
   * @param srid
