...
import org.openrdf.sail.generaldb.schema.*;
...
public abstract class GeneralDBTripleRepository {
  ...
  public GeneralDBResourceIteration findContexts() throws SQLException, RdbmsException {
    flush();
  ...
  protected synchronized void acquireLock() throws InterruptedException {
    if (readLock == null) {
  ...
  protected String buildContextQuery() throws SQLException {
  ...
  protected abstract String buildCountQuery(RdbmsResource... ctxs) throws SQLException;

  protected abstract String buildDeleteQuery(String tableName, RdbmsResource subj, RdbmsURI pred,
      RdbmsValue obj, RdbmsResource... ctxs) throws RdbmsException, SQLException;

  protected GeneralDBSqlQueryBuilder buildSelectQuery(RdbmsResource subj, RdbmsURI pred,
      RdbmsValue obj, RdbmsResource... ctxs) throws RdbmsException, SQLException {
    ...
    query.select().append(
        "CASE WHEN ou.value IS NOT NULL THEN ou.value"
  ...
  protected abstract String buildWhere(StringBuilder sb, RdbmsResource subj, RdbmsURI pred,
      RdbmsValue obj, RdbmsResource... ctxs);

  protected void insert(RdbmsStatement st) throws RdbmsException, SQLException,
  ...
  protected void setCountQuery(PreparedStatement stmt, RdbmsResource... ctxs) throws SQLException,
      RdbmsException {
    if (ctxs != null && ctxs.length > 0) {
  ...
  protected void setSelectQuery(PreparedStatement stmt, RdbmsResource subj, RdbmsURI pred,
      RdbmsValue obj, RdbmsResource... ctxs) throws SQLException, RdbmsException {
    int p = 0;
    if (ctxs != null && ctxs.length > 0) {
      for (int i = 0; i < ctxs.length; i++) {
        if (ctxs[i] == null) {
