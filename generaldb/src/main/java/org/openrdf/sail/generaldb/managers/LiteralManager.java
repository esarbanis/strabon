...
public class LiteralManager extends ValueManagerBase<RdbmsLiteral> {
  ...
  // LoggerFactory.getLogger(org.openrdf.sail.generaldb.managers.LiteralManager.class);
  ...
  @Override
  protected void insert(Number id, RdbmsLiteral literal) throws NullPointerException, SQLException,
    ...
    if (datatype == null && language == null) {
      ...
      try {
        if (XMLGSDatatypeUtil.isNumericDatatype(datatype)) {
          ...
          /**
           * FIXME NOTE: Cannot support the intervalStart and intervalEnd here!! Will need some
           * other place to add them if this approach does work
           * 
           */
