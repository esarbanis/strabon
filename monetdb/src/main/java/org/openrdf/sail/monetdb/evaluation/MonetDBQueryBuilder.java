...
public class MonetDBQueryBuilder extends GeneralDBQueryBuilder {
  ...
  public enum SpatialOperandsMonetDB {
    left, right, above, below;
  }
  public enum SpatialFunctionsMonetDB { // Spatial Relationships
    ST_Disjoint, ST_Touches, ST_Crosses, ST_Within, ST_Overlaps, ST_Relate,

    // These Spatial Relations are implemented in MonetDB as operands and they apply in MBB of a
    // geometry
    mbbIntersects, equals, contains, inside,
  ...
  protected void append(GeneralDBSqlNull expr, GeneralDBSqlExprBuilder filter) {
    ...
    if (parent instanceof GeneralDBSqlOr || parent instanceof GeneralDBSqlAnd) {
      before = " CAST(";
      after = " AS boolean) ";
    }
  ...
  public GeneralDBQueryBuilder construct(GeneralDBSqlExpr expr)
    ...
    if (expr instanceof BinaryGeneralDBOperator) {
      dispatchBinarySqlOperator((BinaryGeneralDBOperator) expr,
          (MonetDBSqlExprBuilder) query.select);
   ...
   * org.openrdf.sail.postgis.evaluation.MonetDBQueryBuilder.append#(org.openrdf.sail.generaldb.
  ...
  protected String appendWKT(GeneralDBSqlExpr expr, GeneralDBSqlExprBuilder filter) {
    ...
    filter.append(" GeomFromText('" + wkt.getWKT() + "',"
        + String.valueOf(GeoConstants.defaultSRID) + ")");
  ...
  protected void appendGeneralDBDateTimeFunctionBinary(BinaryGeneralDBOperator expr,
    ...
    if (check1) {
      ...
      else if (expr.getLeftArg() instanceof GeneralDBLabelColumn
          && !((GeneralDBLabelColumn) expr.getLeftArg()).isSpatial()) {
  ...
  // EXCEPT ST_Transform!!!
  protected void appendMonetDBSpatialFunctionBinary(BinaryGeneralDBOperator expr,
      GeneralDBSqlExprBuilder filter, SpatialFunctionsMonetDB func)
      throws UnsupportedRdbmsOperatorException {
    // In the case where no variable is present in the expression! e.g
    // ConvexHull("POLYGON((.....))")
    boolean sridNeeded = true;
    ...
    boolean check1 =
        expr.getLeftArg().getClass().getCanonicalName()
            .equals("org.openrdf.sail.monetdb.algebra.MonetDBSqlNull");
    if (check1) {
      this.append((GeneralDBSqlNull) expr.getLeftArg(), filter);
      ...
      // XXX Careful: ST_Transform support MISSING!!!
        case ST_Difference:
          filter.appendFunction("Difference");
          break;
        case ST_Intersection:
          filter.appendFunction("Intersection");
          break;
        case ST_Union:
          filter.appendFunction("\"Union\"");
          break;
        case ST_SymDifference:
          filter.appendFunction("SymDifference");
          break;
        case ST_Touches:
          filter.appendFunction("Touches");
          break;
        case ST_Disjoint:
          filter.appendFunction("Disjoint");
          break;
        case ST_Crosses:
          filter.appendFunction("Crosses");
          break;
        case ST_Overlaps:
          filter.appendFunction("Overlaps");
          break;
        case ST_Within:
          filter.appendFunction("Within");
          break;
        case mbbIntersects:
          filter.appendFunction("NOT Disjoint");
          break;
        case equals:
          filter.appendFunction("Equals");
          break;
        case contains:
          filter.appendFunction("Contains");
          break;
        case inside:
          filter.appendFunction("Within");
          break;
  ...
  protected void appendMonetDBDistance(TripleGeneralDBOperator expr,
      GeneralDBSqlExprBuilder filter, SpatialFunctionsMonetDB func)
      throws UnsupportedRdbmsOperatorException {
    String units = null;
  ...
  protected void appendMonetDBBuffer(TripleGeneralDBOperator expr, GeneralDBSqlExprBuilder filter,
      SpatialFunctionsMonetDB func) throws UnsupportedRdbmsOperatorException {
    boolean sridNeeded = true;
    // XXX Incorporating SRID
    String sridExpr = null;
    String units = null;
  ...
  protected void appendMonetDBSpatialFunctionUnary(UnaryGeneralDBOperator expr,
      GeneralDBSqlExprBuilder filter, SpatialFunctionsMonetDB func)
      throws UnsupportedRdbmsOperatorException {
    // In the case where no variable is present in the expression! e.g
    // ConvexHull("POLYGON((.....))")
    ...
    if (check1) {
      ...
      switch (func) {
        case ST_Envelope:
          filter.appendFunction("Envelope");
          break;
        case ST_ConvexHull:
          filter.appendFunction("ConvexHull");
          break;
        case ST_Boundary:
          filter.appendFunction("Boundary");
          break;
        case ST_Area:
          filter.appendFunction("Area");
          break;
        case ST_Dimension:
          filter.appendFunction("Dimension");
          break;
        case ST_GeometryType:
          filter.appendFunction("GeometryTypeId");
          break;
        case ST_AsText:
          break; // Do nothing MonetDB by default returns geometris in WKT format
        case ST_SRID:
          filter.appendFunction("SRID");
          break;
        case ST_IsEmpty:
          filter.appendFunction("IsEmpty");
          break;
        case ST_IsSimple:
          filter.appendFunction("IsSimple");
          break;
      }
      filter.openBracket();
      if (expr.getArg() instanceof GeneralDBStringValue) {
  ...
  // Used in all the generaldb boolean spatial functions of the form ST_Function(?GEO1,?GEO2)
  protected void appendMonetDBSpatialFunctionTriple(TripleGeneralDBOperator expr,
      GeneralDBSqlExprBuilder filter, SpatialFunctionsMonetDB func)
      throws UnsupportedRdbmsOperatorException {
    filter.openBracket();
    ...
    if (check1) {
      ...
      boolean check2 =
          expr.getRightArg().getClass().getCanonicalName()
              .equals("org.openrdf.sail.monetdb.algebra.GeneralDBSqlNull");
      if (check2) {
        this.append((GeneralDBSqlNull) expr.getRightArg(), filter);
      } else {
        if (expr.getRightArg() instanceof GeneralDBStringValue) {
