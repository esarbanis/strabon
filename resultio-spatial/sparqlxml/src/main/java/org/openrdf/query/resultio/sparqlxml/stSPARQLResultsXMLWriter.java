...
import static org.openrdf.query.resultio.sparqlxml.SPARQLResultsXMLConstants.*;
...
public class stSPARQLResultsXMLWriter implements TupleQueryResultWriter {
  ...
  /**
   * Enables/disables addition of indentation characters and newlines in the XML document. By
   * default, pretty-printing is set to <tt>true</tt>. If set to <tt>false</tt>, no indentation and
   * newlines are added to the XML document. This method has to be used before writing starts (that
   * is, before {@link #startQueryResult(List)} is called).
   */
  ...
  private void writeValue(Value value) throws IOException {
    ...
    } else if (value instanceof StrabonPolyhedron) { // spatial case from new geometry construction
                                                     // (SELECT)
      StrabonPolyhedron poly = (StrabonPolyhedron) value;
      Literal literal =
          new LiteralImpl(WKTHelper.createWKT(poly.stringValue(), poly.getGeometry().getSRID(),
              poly.getGeometryDatatype().toString()), new URIImpl(poly.getGeometryDatatype()
              .toString()));
